...
export default function ConsumirProduto() {
  ...
  const handleCheckboxChange = (produtonome: string) => {
    const isSelected = produtosSelecionados.includes(produtonome);

    if (isSelected) {
      setProdutosSelecionados(produtosSelecionados.filter((id) => id !== produtonome));
    } else {
  ...
  const produtonome = produtosSelecionados[0]
  const clientecpf = cpf

  console.log(petnome, clientecpf, produtonome);

  // consumir produto
  const consumirProduto = () => {
    if (petSelecionado && produtosSelecionados.length > 0) {
      const petnome = petSelecionado.petnome;
      const produtonome = produtosSelecionados[0];
  
      axios
        .post("http://localhost:3001/consumirProduto", {
          clienteCpf: cpf,
          nomeProduto: produtonome,
          nomePet: petnome,
        }).then((response)=>{
          if(response.data.message === 'Produto consumido com sucesso'){
            Swal.fire("Sucesso!", response.data.message, "success");
            
          }
          if(response.data.error === 'Erro ao consumir produto')
          Swal.fire({
            icon: "error",
            title: "Erro",
            ...
            <tr key={produto.produtoid}>
              <th scope="row">
                <input
                  ...
                  onChange={() => handleCheckboxChange(produto.produtonome)}
                  checked={produtosSelecionados.includes(produto.produtonome)}
                />
              </th>
