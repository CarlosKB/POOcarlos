...
export default function ServicoFormCadastro() {
  ...
  const handleClickButton = async () => {
    const response = await axios.post("http://localhost:3001/cadastrarServico", {
        nomeServico: values.nome,
        precoServico: values.preco
      }).then((response)=>{
          if(response.data.message === 'Serviço cadastrado com sucesso'){
            Swal.fire("Sucesso!", response.data.message, "success");
            limparCampos()
          }
          if(response.data.error === 'Erro ao cadastrar o serviço')
          Swal.fire({
            icon: "error",
            title: "Erro",
