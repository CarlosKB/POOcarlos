...
function listarTelefonesPorCPF(clienteCPF, callback) {
  obterClienteIDPorCPF(clienteCPF, (err, clienteID) => {
    if (err) {
      ...
      cliente.query(
        "SELECT ClienteTelefoneID, TelefoneDDD, TelefoneNumero FROM ClienteTelefone WHERE ClienteID = $1",
        [clienteID],
        (err, result) => {
          if (err) {
            console.log(err);
...
function listarPetsPorCPF(cpf, callback) {
  obterClienteIDPorCPF(cpf, (err, clienteID) => {
    if (err) {
      ...
      cliente.query(
        `SELECT PetID, PetNome, PetTipo, PetRaca, PetGenero FROM Pets WHERE ClienteID = $1`,
        [clienteID],
        (err, result) => {
          if (err) {
            console.log(err);
